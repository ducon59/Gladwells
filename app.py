...
def _gdrive_folder_latest_file(folder_id: str) -> tuple[str, str]:
    """
    List files in a public Google Drive folder and return (file_id, filename)
    of the most recently modified .xlsm / .xlsx file.
    Uses the Drive API v3 public endpoint — no auth required for public folders.
    """
    ...
    params = {
        ...
        "pageSize": 20,
        # key is not required for public folders shared with "Anyone with the link"
    }
    resp = requests.get(url, params=params, timeout=15)
    # If the API call fails (quota / auth), fall back to HTML scraping
    if resp.status_code != 200:
        raise RuntimeError(
            f"Drive API returned {resp.status_code}. "
    ...
    if not spreadsheets:
        raise RuntimeError("No spreadsheet files found in the Google Drive folder.")
    latest = spreadsheets[0]   # already sorted by modifiedTime desc
