...
def _gdrive_folder_latest_file(folder_id: str) -> tuple[str, str]:
    """
    Find the most-recently-added spreadsheet in a public Google Drive folder.

    Strategy — tries three methods in order:
      1. embeddedfolderview  (Google's own embed endpoint, no auth needed)
      2. Drive folder HTML page scrape
      3. Direct file download attempt using the folder ID itself as a file ID
         (handles the case where the user accidentally pastes a file ID)
    Returns (file_id, filename).
    """
    spreadsheet_exts = (".xlsm", ".xlsx", ".xls")

    # ── Method 1: embeddedfolderview ─────────────────────────────────────────
    # Google serves clean JSON-ish HTML here for any publicly shared folder.
    ...
    try:
        ...
        if r.status_code == 200 and "docid" in r.text:
            # Each file entry looks like:  "docid":"FILE_ID" ... "title":"name.xlsm"
            entries = re.findall(
                r'"docid"\s*:\s*"([A-Za-z0-9_-]{25,44})"'
                r'(?:(?!docid).){0,400}?"title"\s*:\s*"([^"]+)"',
                r.text, re.DOTALL
            )
            spreadsheets = [
                (fid, name) for fid, name in entries
                if name.lower().endswith(spreadsheet_exts)
            ]
            if spreadsheets:
                # embeddedfolderview lists newest-first
                file_id, filename = spreadsheets[0]
                return file_id, filename
    except requests.RequestException:
        pass  # fall through to method 2

    # ── Method 2: Folder HTML page ───────────────────────────────────────────
    try:
        folder_url = f"https://drive.google.com/drive/folders/{folder_id}"
        r2 = requests.get(folder_url, headers=headers, timeout=20)
        if r2.status_code == 200:
            html = r2.text
            # Google inlines JSON data — look for (name, id) pairs
            pairs = re.findall(
                r'"([^"]{1,120}\.(?:xlsm|xlsx|xls))"'
                r'(?:(?!").){0,300}?"([A-Za-z0-9_-]{25,44})"',
                html, re.IGNORECASE | re.DOTALL
            )
            if pairs:
                filename, file_id = pairs[0]
                return file_id.strip(), filename.strip()

            # Last resort within method 2: grab first Drive file link
            id_hits   = re.findall(r'/file/d/([A-Za-z0-9_-]{25,44})/', html)
            ...
            if id_hits and name_hits:
                return id_hits[0], name_hits[0]
    except requests.RequestException:
        pass

    raise RuntimeError(
        "Could not list files in the Google Drive folder. "
        "Please check:\n"
        "1. The folder is shared as \'Anyone with the link → Viewer\'\n"
        "2. The ID is a *folder* ID (from a URL ending in /folders/ID), "
        "not a file ID\n"
...
with st.sidebar:
    ...
    # Admin override: allow pasting a folder ID in case secrets aren't set
    if not GDRIVE_FOLDER_ID:
        manual_folder = st.text_input(
            "Google Drive Folder ID",
            help="One-time setup: paste the folder ID from the Drive URL. "
                 "Add GDRIVE_FOLDER_ID to Streamlit secrets to remove this prompt."
        )
    else:
        manual_folder = ""
        st.success("✅ Connected to Google Drive folder", icon="☁️")

    if st.button("🔄 Check for new file", use_container_width=True):
        st.cache_data.clear()
    ...
    uploaded = st.file_uploader(
        ...
        type=["xlsm", "xlsx"],
        help="Optional: upload a file directly to override the Drive folder."
    )
    st.divider()
    st.caption("The dashboard always loads the most recently added file from the shared Drive folder.")
    st.caption("To update: simply upload the new monthly file into the folder — no other action needed.")
