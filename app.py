...
def _gdrive_download_file(file_id: str) -> bytes:
    """
    Download a Drive file by ID. Handles three cases Google may return:
      1. Direct binary download (small files)
      2. HTML confirmation page (large-file virus-scan warning) — older pattern
      3. JSON redirect page — newer Google Drive pattern with uuid/download_id tokens
    Raises RuntimeError with a clear message if the result looks like HTML.
    """
    ...
    def _fetch(url, params):
        r = session.get(url, params=params, stream=True, timeout=60, allow_redirects=True)
        r.raise_for_status()
        return r

    # ── Attempt 1: standard /uc download endpoint ─────────────────────────────
    r = _fetch("https://drive.google.com/uc", {"id": file_id, "export": "download"})
    ct = r.headers.get("Content-Type", "")

    if "text/html" in ct or "text/html" in r.headers.get("content-type", ""):
        html = r.text

        # Pattern A: older confirmation token in URL query string
        token = re.search(r'[?&]confirm=([0-9A-Za-z_-]+)', html)
        if token:
            r = _fetch("https://drive.google.com/uc",
                       {"id": file_id, "export": "download", "confirm": token.group(1)})
            ct = r.headers.get("Content-Type", "")

        # Pattern B: newer JSON blob with uuid + download_id
        if "text/html" in ct:
            uuid_m     = re.search(r'"uuid"\s*:\s*"([^"]+)"', html)
            dlid_m     = re.search(r'"download_id"\s*:\s*"([^"]+)"', html)
            if uuid_m and dlid_m:
                r = _fetch("https://drive.usercontent.google.com/download", {
                    "id": file_id,
                    "export": "download",
                    "confirm": "t",
                    "uuid": uuid_m.group(1),
        ...
        # Pattern C: usercontent direct endpoint (works for many public files)
        if "text/html" in ct:
            r = _fetch("https://drive.usercontent.google.com/download",
                       {"id": file_id, "export": "download", "confirm": "t"})
            ct = r.headers.get("Content-Type", "")

    # ── Final check: make sure we got a binary file, not an HTML page ─────────
    raw = b"".join(chunk for chunk in r.iter_content(chunk_size=32768) if chunk)

    # Office files start with PK (zip/xlsx/xlsm) or D0CF (old .xls compound doc)
    is_office = raw[:2] == b"PK" or raw[:8] == b"\xd0\xcf\x11\xe0\xa1\xb1\x1a\xe1"
    if not is_office:
        # Peek at what we actually got to give a useful error
        preview = raw[:300].decode("utf-8", errors="replace")
        if "accounts.google.com" in preview or "signin" in preview.lower():
            raise RuntimeError(
                "Google is redirecting to a login page. "
        ...
        if "<html" in preview.lower() or "<!doctype" in preview.lower():
            raise RuntimeError(
                ...
                "Check:\n"
                "  1. The file is shared as \'Anyone with the link → Viewer\'\n"
                "  2. The file ID in cell A1 of your index sheet is correct\n"
                f"  File ID attempted: {file_id}\n"
                f"  Response preview: {preview[:120]}"
            )
        raise RuntimeError(
            f"Downloaded data does not look like an Office file "
