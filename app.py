...
def load_from_gdrive(file_id_or_url: str):
    file_id = extract_file_id(file_id_or_url)
    raw_bytes = _gdrive_download(file_id)
...
with st.sidebar:
    ...
    if source == "☁️ Google Drive (auto)":
        gdrive_id = st.text_input(
            ...
            help="Paste the full share URL or just the ID from: drive.google.com/file/d/**THIS_PART**/view"
        )
    ...
    st.caption("Upload new monthly file to same Google Drive location and click Refresh.")
...
if buf is None:
    ...
    <div style="text-align:center; padding: 60px 20px; color: #888;">
      ...
      <p style="font-size:16px; margin-top:12px;">Connect your Google Drive file or upload the monthly .xlsm to get started.</p>
      ...
      3. Paste the full URL or File ID in the sidebar</p>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
